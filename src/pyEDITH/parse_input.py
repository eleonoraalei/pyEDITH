...
def parse_parameters(parameters: dict) -> dict:
    ...
    wavelength_params = ['resolution', 'snr', 'throughput', 'npix_multiplier', 
                        'dark_current', 'read_noise', 'read_time', 'cic']

    parsed_params.update({key: parse_list_param(key, parsed_params['nlambda']) for key in list(set(wavelength_params) &set(parameters.keys()))})
    
    #------ ARRAYS OF LENGTH NTARGS ------              
    target_params = ['Lstar', 'distance', 'magV', 'angdiam', 'nzodis', 'ra', 'dec', 
                    'delta_mag_min']
    
    parsed_params.update({key: parse_list_param(key, parsed_params['ntargs']) for key in list(set(target_params) &set(parameters.keys()))})
   

    #----- ARRAYS OF LENGTH NLAMBDA x NTARGS ----
    if 'mag' in parameters.keys():
        parsed_params['mag'] = [parse_list_param('mag', parsed_params['nlambda']) for targs in range(parsed_params['ntargs'])]
    ...
    if 'separation' in parameters.keys():
        parsed_params['sp'] = [[parse_list_param('separation', parsed_params['ntargs'])]]
    if 'delta_mag' in parameters.keys():
        parsed_params['delta_mag'] =  [[parse_list_param('delta_mag', parsed_params['ntargs'])]]


    #----- SCALARS ----
    scalar_params = ['photap_rad', 'diameter', 'toverhead_fixed', 'toverhead_multi', 
                     'IWA', 'OWA', 'contrast', 'noisefloor_factor', 'bandwidth', 
...
def read_configuration(input_file: Union[Path, str],secondary_flag=False) -> Tuple[Dict, Dict]:
    """
