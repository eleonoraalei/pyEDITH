...
class MockTelescope(Telescope):
    def load_configuration(self, parameters, mediator):
        ...
        self.telescope_throughput= u.Quantity([0.823],DIMENSIONLESS)
        self.temperature= 290.*u.K
        self.Tcontam= 0.95* DIMENSIONLESS
...
class MockDetector(Detector):
    def load_configuration(self, parameters,mediator):
        ...
        self.npix_multiplier= u.Quantity([1.],DIMENSIONLESS)
        self.DC = u.Quantity([3.e-05] ,DARK_CURRENT)
        self.RN = u.Quantity([0.], READ_NOISE)
        self.tread = u.Quantity([1000.] ,READ_TIME)
...
class MockCoronagraph(Coronagraph):
    def load_configuration(self, parameters,mediator):
        self.path=None
        self.keyword='ToyModel'
        self.pixscale= 30. *LAMBDA_D
        self.minimum_IWA= 1. *LAMBDA_D
        self.maximum_OWA=60.*LAMBDA_D
        ...
        self.nrolls=1
        self.nchannels= 2
        self.coronagraph_throughput= u.Quantity([0.44],DIMENSIONLESS)
        self.coronagraph_spectral_resolution=1.*DIMENSIONLESS
        self.npsfratios= 1
        self.npix =4
        self.xcenter= 2. *PIXEL
        self.ycenter= 2. *PIXEL
        self.r= u.Quantity([[63.63961031, 47.4341649 , 47.4341649 , 63.63961031],
            [47.4341649 , 21.21320344, 21.21320344, 47.4341649 ],
            [47.4341649 , 21.21320344, 21.21320344, 47.4341649 ],
            [63.63961031, 47.4341649 , 47.4341649 , 63.63961031]],LAMBDA_D),
        self.omega_lod= u.Quantity([[[2.26980069],
             [2.26980069],
             [2.26980069],
             [2.26980069]],
        ...
        self.skytrans=u.Quantity([[0.65, 0.65, 0.65, 0.65],
            [0.65, 0.65, 0.65, 0.65],
            [0.65, 0.65, 0.65, 0.65],
            [0.65, 0.65, 0.65, 0.65]], DIMENSIONLESS)
        self.photap_frac=u.Quantity([[[0.       ],
             [0.2968371],
             [0.2968371],
             [0.       ]],
        ...
        self.Istar= u.Quantity([[1.70625e-15, 1.70625e-15, 1.70625e-15, 1.70625e-15],
            [1.70625e-15, 1.70625e-15, 1.70625e-15, 1.70625e-15],
            [1.70625e-15, 1.70625e-15, 1.70625e-15, 1.70625e-15],
            [1.70625e-15, 1.70625e-15, 1.70625e-15, 1.70625e-15]], DIMENSIONLESS)
        self.noisefloor= u.Quantity([[5.11875e-17, 5.11875e-17, 5.11875e-17, 5.11875e-17],
...
def mock_observation():
    obs = Observation()
    obs.td_limit= 1.e+20 *u.s
    obs.wavelength=u.Quantity([0.5],u.micron)
    obs.SNR = u.Quantity([7],DIMENSIONLESS)
    obs.photap_rad = 0.85*LAMBDA_D
    obs.CRb_multiplier = 2
    obs.nlambd = 1
    obs.tp = 0.*u.s 
    obs.exptime = u.Quantity([0.],u.s)
    obs.fullsnr = u.Quantity([0.],DIMENSIONLESS)
...
def mock_scene():
    ...
    scene.Fstar=u.Quantity([0.00334326],DIMENSIONLESS)
    scene.Fp0=u.Quantity([6.30957344e-11],DIMENSIONLESS)
    scene.Fp0_min = 1.e-10*DIMENSIONLESS
    scene.angular_diameter_arcsec=0.01*ARCSEC
    scene.nzodis=3*ZODI 
...
def test_observatory_validate_configuration(mock_observatory):
    ...
    with pytest.raises(AttributeError, match="Observatory is missing attribute: optics_throughput"):
        mock_observatory.validate_configuration()

    mock_observatory.optics_throughput = 0.8  # Not a Quantity
    with pytest.raises(TypeError, match="Observatory attribute optics_throughput should be a Quantity"):
        mock_observatory.validate_configuration()

    mock_observatory.optics_throughput = [0.8] * u.meter  # Wrong unit
    with pytest.raises(ValueError, match="Observatory attribute optics_throughput has incorrect units"):
        mock_observatory.validate_configuration()


def test_calculate_optics_throughput(mock_observatory):
    """Test calculation of optics throughput"""
    # Test with Toptical parameter
    parameters = {
        "Toptical": 0.8,
    ...
    # Test without Toptical parameter
    parameters = {"observing_mode": "IMAGER"}
    mock_observatory.calculate_optics_throughput(parameters)
    expected = mock_observatory.telescope.telescope_throughput.value[0] * mock_observatory.coronagraph.coronagraph_throughput.value[0]
    assert mock_observatory.optics_throughput.value == expected

def test_calculate_warmemissivity_coldtransmission(mock_observatory):
...
def test_observatory_mediator(mock_observatory, mock_observation, mock_scene):
    ...
    assert mediator.get_telescope_parameter('diameter') == mock_observatory.telescope.diameter

    # Test getting coronagraph parameter
    assert mediator.get_coronagraph_parameter('contrast') == mock_observatory.coronagraph.contrast

    # Test getting detector parameter
    assert mediator.get_detector_parameter('pixscale_mas') == mock_observatory.detector.pixscale_mas

    # Test getting observation parameter
    assert mediator.get_observation_parameter('wavelength') == mock_observation.wavelength
