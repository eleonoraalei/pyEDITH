...
def calc_Blambda_chris(temp, lam):
    ...
    result = const1 / (
        (lam_cm * lam_cm * lam_cm * lam_cm * lam_cm)
        * (np.exp(const2 / (lam_cm * temp)) - 1.0)
    )

    result /= (
        hc / lam_cm
    )  # divide by energy of a photon to get photons cm^-3 s^-1 steradian^-1
...
def calc_CRbthermal():
    ...
    +++add together into tempCRbfactor+++
    CRb = tempCRbfactor * omega_lod[index2];
    """
...
CRbthermalfactor = (
    Blam_chris * deltalambda_nm * A_cm * (lod_rad * lod_rad) * epswarmTrcold * QE
)
